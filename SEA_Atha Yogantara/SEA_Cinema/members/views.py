...
def homepage(request):
    ...
    if login_username : 
        ...
        user = ''
    return render(request,'main.html',{'movie' : movie,'user' : user})
...
def login(request):
    if request.method == 'POST' : 
        ...
        if UserData.objects.filter(username = username,password=password).exists() : 
            ...
            messages.error(request,'Username and/or password incorrect. Please verify and try again.')
...
def register(request):
    if request.method == 'POST' : 
        ...
        if not UserData.objects.filter(username = username).exists() : 
            ...
            messages.success (request,'Registration Succsessfully')
        else : 
...
def profile(request) : 
    ...
    return render(request,'YourAccount.html',{'user' : user,
                                          'tickets' : tickets})
    ...
    if request.method == 'POST':
        ...
        if len(selected_seats) > 0 : 
            if user.age < movie.age_rating : 
                messages.error(request,"Age is just a number, but patience is a virtue. I'll eagerly await the day when I'm old enough to enjoy this film !")
            elif user.balance == None or user.balance < movie.ticket_price * len(selected_seats): 
                messages.error(request,'Insufficient balance')
            elif len(selected_seats) > 6 : 
                messages.error(request,'Limited to a maximum purchase of 6 tickets per Account')
            else :
                ...
                success_message = 'TTicket successfully booked !'
